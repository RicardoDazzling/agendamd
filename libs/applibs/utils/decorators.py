...
def ignore_args(func):
    @wraps(func)
    def ignore_args_wrapper_func(*args, **kwargs):
        if args:
            pass
        __search = getattr(args[0], func.__name__, None)
        __return = func(**kwargs) if __search is None else func(args[0], **kwargs)
        if __return:
            return __return
...
def ignore_kwargs(func):
    @wraps(func)
    def ignore_kwargs_wrapper_func(*args, **kwargs):
        if kwargs:
            pass
        __return = func(*args)
        if __return:
...
def ignore_instance(func):
    @wraps(func)
    def ignore_instance_wrapper_func(*args, **kwargs):
        __search = getattr(args[0], func.__name__, None)
        __value = getattr(args[-1], func.__name__, None)
        args = list(args)
        args.pop(0) if __search is None or (len(args) <= 2 and __value is not None) else args.pop(1)
...
def only_the_instance(func):
    @wraps(func)
    def only_the_instance_wrapper_func(*args, **kwargs):
        __search = getattr(args[0], func.__name__, None)
